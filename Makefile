...
docker-build: verify-apis test
	...
	docker run --rm -v $(TEMP_DIR):/build -v $(shell pwd):/go/src/https://github.com/Comedycom/k8s-cloudwatch-adapter -e GOARCH=$(ARCH) -e GOFLAGS="$(GOFLAGS)" -w /go/src/https://github.com/Comedycom/k8s-cloudwatch-adapter $(GOIMAGE) /bin/bash -c "\
